...
# for i in range(len(countyInfo)):
#   for j in range(len(countyInfo[i])):
#     versionUrl = urlFront + countyInfo[0][0]
#     print(versionUrl)


# for i in range(len(countyInfo)):
#   for j in range(len(countyInfo[i])):
#     print(countyInfo[i][j])

# for key, value in countyInfo.items():
#     versionUrl = urlFront + countyInfo['county']['countyName'] + '/' + countyInfo['county']['countyNumber'] + versionUrlBack
#     versionNumber = (requests.get(versionUrl, headers=headers)).content
#     dataUrl = urlFront + countyInfo['county']['countyName'] + '/' +  countyInfo['county']['countyNumber'] + '/' + versionNumber + dataUrlBack
#     countyName = countyInfo['county']['countyName']
#     resultsData = (requests.get(dataUrl, headers=headers)).content
#     with open(countyName + '_data.json', 'w') as f:
#       f.write(resultsData)

#print(countyInfo[0])
#print(key, value)

# doloresVersionUrl = 'https://results.enr.clarityelections.com/CO/Dolores/115922/current_ver.txt'
# headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36'}

# doloresVersion = (requests.get(versionUrl, headers=headers)).content
# doloresUrl = 'https://results.enr.clarityelections.com/CO/Dolores/115922/' + doloresVersion + '/json/sum.json'
# doloresData = (requests.get(doloresUrl, headers=headers)).content

# with open("dolores_data.json", "w") as f:
#     f.write(doloresData)

#response = requests.get(url, headers=headers)
#print(response.content)
#print(doloresData.content)